...
import com.google.common.util.concurrent.ForkedSmoothRateLimiter.ForkedSmoothBursty;
import com.google.common.util.concurrent.ForkedSmoothRateLimiter.SmoothWarmingUp;
...
public abstract class ForkedRateLimiter {
    ...
    public static ForkedRateLimiter create(double permitsPerSecond, Ticker ticker) {
        return create(SleepingStopwatch.createFromTicker(ticker), permitsPerSecond);
    }
     ...
     */
    @VisibleForTesting
    static ForkedRateLimiter create(SleepingStopwatch stopwatch, double permitsPerSecond) {
        ForkedRateLimiter reRateLimiter = new ForkedSmoothBursty(stopwatch, 1.0 /* maxBurstSeconds */);
        reRateLimiter.setRate(permitsPerSecond);
        return reRateLimiter;
    }
     ...
     */
    public static ForkedRateLimiter create(double permitsPerSecond, long warmupPeriod, TimeUnit unit) {
    ...
    @VisibleForTesting
    static ForkedRateLimiter create(
            SleepingStopwatch stopwatch, double permitsPerSecond, long warmupPeriod, TimeUnit unit,
            double coldFactor) {
        ForkedRateLimiter reRateLimiter = new SmoothWarmingUp(stopwatch, warmupPeriod, unit, coldFactor);
        reRateLimiter.setRate(permitsPerSecond);
        return reRateLimiter;
    }
    ...
    ForkedRateLimiter(SleepingStopwatch stopwatch) {
    ...
    public String toString() {
        return String.format(Locale.ROOT, "[stableRate=%3.1fqps]", getRate());
    }
    ...
    public static final ForkedRateLimiter ALWAYS_REJECT = new ForkedRateLimiter(new SleepingStopwatch() {
        @Override
        long readMicros() {
            return 0;
        }
        ...
        void doSetRate(double permitsPerSecond, long nowMicros) {
            if (permitsPerSecond != 0.0) {
                throw new UnsupportedOperationException("Only valid rate is 0.0.");
            }
        }
        ...
        public double acquire(int permits) {
            if (permits == 0) {
                ...
                throw new UnsupportedOperationException("Rate is zero so refusing to sleep forever.");
            }
        }
